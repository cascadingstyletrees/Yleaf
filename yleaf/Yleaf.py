...
CACHED_POSITION_DATA: set[str] | None = None
...
def main_vcf(args: argparse.Namespace, base_out_folder: Path):
    position_bed_file = get_position_bed_file(
        args.reference_genome, args.use_old, args.ancient_DNA
    )
    path_markerfile = get_position_file(
        args.reference_genome, args.use_old, args.ancient_DNA
    )
    ...
    if not args.reanalyze:
        with multiprocessing.Pool(processes=args.threads) as p:
            sample_vcf_files = p.map(
                partial(main_vcf_split, position_bed_file, base_out_folder, args), files
            )
        ...
        with multiprocessing.Pool(processes=args.threads) as p:
            p.map(
                partial(run_vcf, path_markerfile, base_out_folder, args),
                sample_vcf_files,
            )
...
def main_fastq(args: argparse.Namespace, out_folder: Path):
...
def main_bam_cram(args: argparse.Namespace, base_out_folder: Path, is_bam: bool):
...
def call_command(command_str: str, stdout_location: TextIO = None):
    ...
    if stdout_location is None:
        ...
        process = subprocess.Popen(
            command_str, stderr=subprocess.PIPE, stdout=stdout_location, shell=True
        )
    # blocking call
    stdout, stderr = process.communicate()
    ...
    if process.returncode != 0:
        LOG.error(
            f"Call: '{command_str}' failed. Reason given: '{stderr.decode('utf-8')}'"
        )
        raise SystemExit("Failed command execution")
    LOG.debug(f"Finished running the command {command_str}")


def get_files_with_extension(path: str | Path, ext: str) -> list[Path]:
...
def get_reference_path(requested_version: str, is_full: bool) -> Path | None:
    if is_full:
...
def samtools(
    ...
    if is_bam_pathfile:
        ...
        if not any(
            ...
            cmd = f"samtools index -@{args.threads} {path_file}"
            call_command(cmd)
    header, mapped, unmapped = chromosome_table(
        path_file, output_folder, output_folder.name
    )

    position_file = get_position_file(
        args.reference_genome, args.use_old, args.ancient_DNA
    )
...
def write_bed_file(bed: Path, markerfile: Path, header: str):
...
def replace_with_bases(base: str, read_result: str) -> str:
...
def find_private_mutations(
    output_folder: Path, path_file: Path, args: argparse.Namespace, is_bam: bool
):
...
def load_filter_data(path: Path) -> set[str]:
...
def load_snp_database_file(
    path: Path, minor_allele_frequency: float
            ...
            with open(path) as f:
                ...
                for line in f:
                    ...
                    if float(frequency) > minor_allele_frequency:
                        continue
                    CACHED_SNP_DATABASE[position].append(
                        {
                            "rs": rs,
...
def load_reference_file(path: Path) -> str | None:
...
def draw_haplogroups(haplogroup_file: Path, collapsed_draw_mode: bool):
